...
class OpenImagesDataset:
    ...
    def __init__(self, root,
        ...
        # self.data_path = "/media/mostafa-ikiwi/data/datasets/openimage"
    ...
    def _read_data(self):
        ...
        class_names = ['BACKGROUND'] + sorted(list(annotations['Class_names'].unique()))
        class_dict = {class_name: i for i, class_name in enumerate(class_names)}
        data = []
        for image_id, group in annotations.groupby("ImageID"):
            boxes = group.loc[:, ["XMin", "YMin", "XMax", "YMax"]].values.astype(np.float32)
            labels = np.array([class_dict[name] for name in group["Class_names"]])
            data.append({
                'image_id': image_id,
                'boxes': boxes,
                'labels': labels
