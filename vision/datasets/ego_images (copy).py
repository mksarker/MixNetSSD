...
class OpenImagesDataset:
    ...
    def _read_data(self):
        ...
        # noun_class_file=f"{self.root}/EPIC_noun_classes.csv"
        # noun_class= pd.read_csv(noun_class_file) 
        # class_names=  ['BACKGROUND'] + sorted(list(noun_class['class_key'].unique())) 
        ...
        for image_id, group in annotations.groupby("image"):   
            boxes = list(group["bounding_box"])
            boxes=boxes[0]
            # boxes=np.matrix(boxes)
            boxes=np.array(boxes)
            # boxes=boxes[1:-1]
            # XMin=boxes[1:-1].split(' ')[0]
            # YMin=boxes[1:-1].split(' ')[1]
            # XMax=boxes[1:-1].split(' ')[2]
            # YMax=boxes[1:-1].split(' ')[3]

            # boxes = [boxes]
            # print(boxes)
            
            # boxes= np.array([boxes[box] for box in boxes])
            # print(type(boxes))
...
#     dataset_path='/media/mostafa/RESEARCH/EPIC_KITCHENS_2018/detection_code/pytorch-ssd/data/ego/'
...
#     image=cv2.putText(image,labels[int(lbl)] ,(x1,y1), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0,255,0),2,cv2.LINE_AA)  
